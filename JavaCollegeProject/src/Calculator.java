...
public class Calculator {
    ...
    Connection conn = DriverManager.getConnection(url , "root" , "096117333Pp#"); public Calculator()throws ClassNotFoundException, SQLException {

        Class.forName("com.mysql.cj.jdbc.Driver"); //new driver itseems
        Statement s = conn.createStatement();
        PreparedStatement pst = conn.prepareStatement("SELECT * FROM calculator_history");
//        ResultSet resul = s.executeQuery("Select * from calculator_history");
        JButton historyButton = new JButton("History");
        historyButton.setBounds(20, 0, 110, 25);
        ResultSet rs = pst.executeQuery();

        historyButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                JFrame historyFrame = new JFrame("Calculator History");
                historyFrame.setSize(480, 300);
                historyFrame.setLocationRelativeTo(null);

                String[] columnNames = {"calculation", "result", "created_at"};
                DefaultTableModel tableModel = new DefaultTableModel(columnNames, 0);
                JTable table = new JTable(tableModel);
                try {
                    Statement stmt = conn.createStatement();
                    ResultSet rs = stmt.executeQuery("SELECT * FROM calculator_history");

                    while (rs.next()) {
                        String calculation = rs.getString("calculation");
                        double result = rs.getDouble("result");
                        String createdAt = rs.getString("created_at");
                        Object[] rowData = {calculation, result, createdAt};
...
// create an ActionListener for the buttons
        ActionListener buttonListener = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                // update the text field with the expression
                inText.setText(expression);
            }
        };
        ...
        // create a String variable to store the expression
        ...
        btnEqual = initBtn("=", x[2], y[5], event -> {
            ...
            String sql = "INSERT INTO calculator_history (calculation, result) VALUES (?, ?)";
            try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                // set the parameters of the prepared statement
                String buttonInfo = event.getActionCommand(); // Get the text of the clicked button
        ...
        btnRoot = initBtn("√", x[4], y[1], event -> {
            if (Pattern.matches("([-]?\\d+[.]\\d*)|(\\d+)", inText.getText()))
                if (go) {
                    double demo = Double.parseDouble(inText.getText());
                    val = Math.sqrt(demo);
                    ok = "√"+demo;
                    if (Pattern.matches("[-]?[\\d]+[.][0]*", String.valueOf(val))) {
                        inText.setText(String.valueOf((int) val));
        ...
        btnPower = initBtn("pw", x[4], y[2], event -> {
            repaintFont();
            if (Pattern.matches("([-]?\\d+[.]\\d*)|(\\d+)", inText.getText()))
                if (go) {
                    val = calc(val, inText.getText(), opt);
        ...
        window.setDefaultCloseOperation(EXIT_ON_CLOSE); // Close button clicked? = End The process
